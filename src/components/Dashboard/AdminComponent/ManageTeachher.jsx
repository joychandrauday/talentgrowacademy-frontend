...
const ManageTeachher = () => {
    ...
    const { teachers, totalCount, isLoading, isError, refetch } = useTeacher(queryParams);
    const { teacherManagers } = useTeacherManager()
    ...
    const handleSave = async (userId) => {
        try {
            ...
            if (confirm) {
                await axiosPublic.patch(`/teachers/${userId}/all`, editedUserData);
    ...
    const handleAssignUser = async (userID) => {
        try {
            ...
            const teacherOptions = teacherManagers?.map(leader =>
            ...
            const { value: formValues } = await Swal.fire({
                ...
                html: `
                    <label for="teachersManagers" style="display: block; text-align: left;">Select Senior Group Leader:</label>
                    <select id="teachersManagers" class="swal2-select" style="width: 100%; padding: 0.5rem;">
                ...
                preConfirm: () => {
                    const teacherManager = document.getElementById('teachersManagers').value;
            ...
            if (formValues) {
                const { teacherManager } = formValues;

                const dataToSend = {};

                if (teacherManagers) {
                    dataToSend.teacherManager = teacherManager;
                }

                if (Object.keys(dataToSend).length > 0) {
                    await axiosPublic.patch(`/teachers/${userID}/all`, dataToSend);
                    refetch();
    ...
    return (
        <div className="p-5">
            <h1 className="text-2xl font-bold text-left mb-5">All Teacers</h1>
                        ...
                        <tbody>
                            {teachers.map((user, index) => (
                                <tr key={user._id} className="hover:bg-gray-50">
                                    ...
                                    <td className="border px-4 py-2">{user.teacherManager?.name || 'N/A'}</td>
                                    <td className="border px-4 py-2">
                                        {editingUserId === user._id ? (
                                            <input
                                                type="text"
