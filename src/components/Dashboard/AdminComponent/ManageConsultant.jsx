...
const ManageConsultant = () => {
    ...
    const { consultants, totalCount, isLoading, isError, refetch } = useConsultant(queryParams);
    const { consultantManagers } = useConsManager()
    ...
    const handleSave = async (userId) => {
        try {
            ...
            if (confirm) {
                await axiosPublic.patch(`/consultants/${userId}`, editedUserData);
    ...
    const activateUser = async (userID) => {
        try {
            ...
            if (confirm) {
                await axiosPublic.patch(`/consultants/${userID}`, {
                    status: 'active',
                });
    ...
    const handleAssignUser = async (userID) => {
        try {
            ...
            const consManagerOptions = consultantManagers?.map(leader =>
            ...
            const { value: formValues } = await Swal.fire({
                ...
                html: `
                    <label for="consManager" style="display: block; text-align: left;">Select Senior Group Leader:</label>
                    <select id="consManager" class="swal2-select" style="width: 100%; padding: 0.5rem;">
                ...
                preConfirm: () => {
                    const consManager = document.getElementById('consManager').value;
            ...
            if (formValues) {
                const { consManager } = formValues;

                const dataToSend = {};

                if (consManager) {
                    dataToSend.
                        consultantManager = consManager;
                }

                if (Object.keys(dataToSend).length > 0) {
                    await axiosPublic.patch(`/consultants/${userID}`, dataToSend);
                        ...
                        <tbody>
                            {consultants.map((user, index) => (
                                <tr key={user._id} className="hover:bg-gray-50">
                                    ...
                                    <td className="border px-4 py-2">{user.consultantManager?.name || 'N/A'}</td>
                                    <td className="border px-4 py-2">
                                        {editingUserId === user._id ? (
                                            <input
                                                type="text"
