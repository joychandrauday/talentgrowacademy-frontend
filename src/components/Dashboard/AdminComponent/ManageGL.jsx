...
const ManageGL = () => {
    ...
    const handleSave = async (userId) => {
        try {
            ...
            if (confirm) {
                await axiosPublic.patch(`/group-leaders/${userId}`, editedUserData);
    ...
    const activateUser = async (userID) => {
        try {
            ...
            if (confirm) {
                await axiosPublic.patch(`/group-leaders/${userID}`, {
                    status: 'active',
                });
    ...
    const handleAssignUser = async (userID) => {
        try {
            ...
            if (formValues) {
                const { seniorGroupLeader, seniorGroupLeaderManager } = formValues;
                ...
                if (Object.keys(dataToSend).length > 0) {
                    await axiosPublic.patch(`/group-leaders/${userID}`, dataToSend);
                    refetch();
                        ...
                        <tbody>
                            {groupLeaders.map((user, index) => (
                                <tr key={user._id} className="hover:bg-gray-50">
                                    ...
                                    <td className="border px-4 py-2">{user.seniorGroupLeader?.name || 'N/A'}</td>
                                    <td className="border px-4 py-2">{user.seniorGroupLeaderManager?.name || 'N/A'}</td>
                                    <td className="border px-4 py-2">
                                        {editingUserId === user._id ? (
                                            <input
                                                type="text"
